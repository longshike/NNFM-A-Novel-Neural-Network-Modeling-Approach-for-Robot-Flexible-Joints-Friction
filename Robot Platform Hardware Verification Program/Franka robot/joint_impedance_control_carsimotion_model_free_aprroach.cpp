// joint_friction_comparison_control.cpp
// Franka机器人关节摩擦模型对比控制程序
#include <array>
#include <atomic>
#include <cmath>
#include <fstream>
...
int main(int argc, char** argv) {
    ...
    std::thread print_thread([print_rate, &print_data, &running, &init_csv]() {
        if (!init_csv) {
            ...
            outFile.open("friction_comparison_data.csv", std::ios::out);
        ...
        while (running) {
            ...
            if (print_data.mutex.try_lock()) {
                if (print_data.has_data) {
                    // 写入扩展的CSV数据
                    std::ofstream outFile;
                    outFile.open("friction_comparison_data.csv", std::ios::app);
    ...
    try {
        ...
        // 开始控制
        std::cout << "Starting friction comparison control..." << std::endl;
    ...
    std::cout << "Friction comparison experiment completed. Data saved to friction_comparison_data.csv" << std::endl;
    return 0;
}