// figure8_friction_comparison_control.cpp
// Franka机器人八字形轨迹摩擦模型对比控制程序
#include <array>
#include <atomic>
#include <cmath>
#include <fstream>
...
int main(int argc, char** argv) {
    ...
    // 设置八字形轨迹参数
    const double amplitude = 0.07;      // 轨迹幅度
    ...
    std::thread print_thread([print_rate, &print_data, &running, &init_csv]() {
        if (!init_csv) {
            ...
            outFile.open("figure8_friction_comparison_data.csv", std::ios::out);
            // 扩展CSV头文件
            outFile << "time,"
        ...
        while (running) {
            ...
            if (print_data.mutex.try_lock()) {
                if (print_data.has_data) {
                    // 写入扩展的CSV数据
                    std::ofstream outFile;
                    outFile.open("figure8_friction_comparison_data.csv", std::ios::app);
    ...
    try {
        ...
        // 八字形轨迹生成回调函数
        auto cartesian_pose_callback = [amplitude, frequency, run_time, start_up_time, 
            ...
            // 计算基础速度
            // 八字形参数方程：x = A*sin(ωt), z = A*sin(2ωt)
            ...
            if (print_data.mutex.try_lock()) {
                ...
                print_data.cartesian_position[1] = 0;  // Y方向保持为0
                print_data.cartesian_position[2] = z_pos;
                print_data.cartesian_velocity[0] = v_x;
                print_data.cartesian_velocity[1] = 0;  // Y方向速度为0
            ...
            if (static_cast<int>(time * 2) % 10 == 0 && 
                static_cast<int>(time * 2) != static_cast<int>((time - period.toSec()) * 2)) {
        ...
        std::cout << "Starting figure-8 friction comparison control..." << std::endl;
        std::cout << "Trajectory parameters:" << std::endl;
        std::cout << "  Amplitude: " << amplitude << " m" << std::endl;
    ...
    std::cout << "Figure-8 friction comparison experiment completed." << std::endl;
    std::cout << "Data saved to figure8_friction_comparison_data.csv" << std::endl;
    return 0;
}